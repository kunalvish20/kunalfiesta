...
export default function OpenFiestaChat() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        if (!dbThreads) {
          console.warn('No threads found in Supabase')
          setThreads([])
          setActiveThreadId(null)
          return
        }
  ...
  useEffect(() => {
    if (chatRef.current && activeThread) {
      // Always load messages, even if empty array
        ...
        const base = {
          id: `${activeThread.id}-${msg.ts || index}-${index}`,
          content: msg.content,
  ...
  // Don't render main content until hydrated to avoid hydration mismatch
  if (!isHydrated) {
    return (
      <div className="min-h-screen w-full relative dark">
        <LaunchScreen backgroundClass={BACKGROUND_STYLES[theme.background].className} dismissed={false} />
      </div>
    );
  }
    ...
    <div className={cn("min-h-screen w-full relative", isDark ? "dark" : "")}> 
      ...
      {isDark ? (
        <div
          ...
          style={{
            background:
              "linear-gradient(0deg, rgba(0,0,0,0.6), rgba(0,0,0,0.6)), radial-gradient(68% 58% at 50% 50%, #3b82f6 0%, #2563eb 16%, #1d4ed8 32%, #1a3a8a 46%, #16386c 60%, #122a53 72%, #0e203c 84%, #0a1629 94%, #040a10 100%), radial-gradient(90% 75% at 50% 50%, rgba(59,130,246,0.06) 0%, rgba(59,130,246,0) 55%), radial-gradient(150% 120% at 8% 8%, rgba(0,0,0,0) 42%, #0b0a0a 82%, #070707 100%), radial-gradient(150% 120% at 92% 92%, rgba(0,0,0,0) 42%, #0b0a0a 82%, #070707 100%), radial-gradient(60% 50% at 50% 60%, rgba(59,130,246,0.06), rgba(0,0,0,0) 60%), #050505",
          }}
        />
      ) : (
        <div className="min-h-screen w-full bg-[#f0f9ff] relative">
          <div
            className="absolute inset-0 z-0"
            style={{
              backgroundImage: `
                radial-gradient(circle at 20% 80%, rgba(191, 219, 254, 0.3) 0%, transparent 50%),
                radial-gradient(circle at 80% 20%, rgba(219, 234, 254, 0.35) 0%, transparent 50%),
                radial-gradient(circle at 40% 40%, rgba(191, 219, 254, 0.15) 0%, transparent 50%)`,
            }}
          />
        </div>
      )}
          ...
          <div className="flex-1 min-w-0 flex flex-col h-[calc(100vh-2rem)] lg:h-[calc(100vh-3rem)] overflow-hidden relative">
            ...
            <div className="lg:hidden flex items-center justify-between p-4 border-b border-white/10">
              ...
              <div className="relative flex items-center gap-2">
                <Link
                  href="/compare"
                  className="inline-block bg-blue-950 text-blue-400 border border-blue-400 border-b-2 font-medium overflow-hidden relative px-2 py-1 rounded-md hover:brightness-150 hover:border-t-2 hover:border-b active:opacity-75 outline-none duration-300 group text-[10px]"
                >
                  <span className="bg-blue-400 shadow-blue-400 absolute -top-[150%] left-0 inline-flex w-40 h-[3px] rounded-md opacity-50 group-hover:top-[150%] duration-500 shadow-[0_0_10px_10px_rgba(0,0,0,0.3)]"></span>
                  Compare Models
                </Link>
                <button
                ...
                {mobileActionsOpen && (
                  <div className="absolute right-0 top-11 z-50 rounded-xl border border-white/15 bg-black/60 backdrop-blur-md shadow-xl p-2 flex items-center gap-2">
                    ...
                    <ThemeToggle compact />
                    <Settings compact />
                    <GithubStar owner="Kunal Vishwakarma" repo="Open-Fiesta" />
                  </div>
                )}
              </div>
            </div>
