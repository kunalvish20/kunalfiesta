        ...
        <header className="mb-6 space-y-4">
          ...
          <nav className="flex items-center justify-between" aria-label="Page navigation">
            <Link
              ...
              aria-label="Return to kunal ai main application"
            >
            ...
            {turn.assistants.length > 0 && (
              <div className="space-y-4">
                {turn.assistants.length > 1 ? (
                  ...
                  <div className="group relative rounded-lg p-3 h-full min-h-[140px] flex overflow-hidden ring-1 transition-shadow bg-gradient-to-b from-blue-900 to-blue-700 ring-blue-700/50 backdrop-blur-[2px] hover:ring-blue-600/80">
                    <div className="text-sm leading-relaxed w-full space-y-2 max-h-[40vh] md:max-h-[400px] overflow-y-auto">
                      {/* Model header */}
                      <div className="flex items-center gap-2 mb-3 pb-2 border-b border-blue-700">
                        <div
                          className="flex-shrink-0 w-6 h-6 bg-blue-700/20 rounded-full flex items-center justify-center"
                          role="img"
                          aria-label="Assistant avatar"
                        >
                          <Bot size={12} className="text-blue-400" aria-hidden="true" />
                        </div>
                      ...
                      {assistant.code && assistant.code >= 400 && (
                        <div className="mt-2 text-xs text-blue-400" role="alert" aria-live="polite">
                      ...
                      {/* Provider info */}
                      {assistant.provider && (
                        <div className="text-xs text-white/40 mt-2 pt-2 border-t border-blue-700">
                  ...
                  turn.assistants.map((assistant, assistantIndex) => (
                    <div key={assistantIndex} className="flex gap-3" role="group" aria-labelledby={`assistant-message-${turnIndex}-${assistantIndex}`}>
                      <div
                        className="flex-shrink-0 w-8 h-8 bg-blue-700/20 rounded-full flex items-center justify-center"
                        role="img"
                        aria-label="Assistant avatar"
                      >
                        <Bot size={16} className="text-blue-400" aria-hidden="true" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          {assistant.provider && (
                            <span className="text-xs text-white/40 capitalize border-b border-blue-700">
                              via {assistant.provider}
                            </span>
                          )}
                        </div>
